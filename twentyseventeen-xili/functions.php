<?php
// dev.xiligroup.com - msc - 2019-05-10 - first implementation with WP 5.2
...
define( 'TWENTYSEVENTEEN_XILI_VER', '2.2' ); // as parent style.css

function twentyseventeen_xilidev_setup() {

	$theme_domain = 'twentyseventeen';
	...
	if ( class_exists( 'xili_language' ) ) { // if temporary disabled
		...
		// Args dedicated to this theme named Twenty Seventeen
		$xili_args = array(
			'customize_clone_widget_containers' => true, // comment or set to true to clone widget containers
			'settings_name' => 'xili_2017_theme_options', // name of array saved in options table
			'theme_name' => 'Twenty Seventeen',
			'theme_domain' => $theme_domain,
			'child_version' => TWENTYSEVENTEEN_XILI_VER,
		);

		add_action( 'widgets_init', 'twentyseventeen_xili_add_widgets' );
		...
		add_theme_support(
			...
			array(
				'post_format' => array(
					...
					'description' => __( 'Will copy post_format in the future translated posts', 'twentyseventeen' ),
				),
				'post_content' => array(
					'default' => '',
					'data' => 'post',
					...
					'name' => 'Post Content',
					/* translators: added in child functions by xili */
					'description' => __( 'Will copy content in the future translated post', 'twentyseventeen' ),
				),
				'post_parent' => array(
					'default' => '1', // (checked)
					'data' => 'post',
					...
					/* translators: added in child functions by xili */
					'description' => __( 'Will copy translated parent id (if original has parent and translated parent)!', 'twentyseventeen' ),
				),
			)
		); //
		...
		if ( $class_ok ) {
			...
			if ( $xl_required_version ) { // msg choice is inside class
				...
				<div class="error">' .
					/* translators: added in child functions by xili */
					'<p>' . sprintf( __( 'The %1$s child theme requires xili_language version more recent than %2$s installed', 'twentyseventeen' ), get_option( 'current_theme' ), $minimum_xl_version ) . '</p>
				</div>';
			...
			<div class="error">' .
				/* translators: added in child functions by xili */
				'<p>' . sprintf( __( 'The %s child theme requires xili_language_theme_options class installed and activated', 'twentyseventeen' ), get_option( 'current_theme' ) ) . '</p>
			</div>';
		}
	} else {
		...
		<div class="error">' .
			/* translators: added in child functions by xili */
			'<p>' . sprintf( __( 'The %s child theme requires xili-language plugin installed and activated', 'twentyseventeen' ), get_option( 'current_theme' ) ) . '</p>
		</div>';
	...
	//remove_filter( 'walker_nav_menu_start_el', 'twentyseventeen_nav_description');

}
add_action( 'after_setup_theme', 'twentyseventeen_xilidev_setup', 11 ); // called after parent
...
function twentyseventeen_xili_add_widgets() {
	register_widget( 'Xili_Widget_Categories' ); // in xili-language-widgets.php since 2.16.3
}

function twentyseventeen_xili_header_image() {
	...
	if ( $header_image_url ) {
		...
		/**
		 * Filter the default twentyseventeen custom header sizes attribute.
		 *
		 ...
		 */
		$custom_header_sizes = apply_filters( 'twentyseventeen_custom_header_sizes', '(max-width: 709px) 85vw, (max-width: 909px) 81vw, (max-width: 1362px) 88vw, 1200px' );
...
function twentyseventeen_xilidev_setup_custom_header() {
	...
	register_default_headers(
		array(
			'xili2017' => array(
				'url' => '%2$s/assets/images/headers/xili2017-h1.jpg',
				'thumbnail_url' => '%2$s/assets/images/headers/xili2017-h1-thumb.jpg',
				/* translators: added in child functions by xili */
				'description' => _x( '2017 by xili', 'header image description', 'twentyseventeen' ),
			),
			'xili2017-2' => array(
				'url' => '%2$s/assets/images/headers/xili2017-h2.jpg',
				'thumbnail_url' => '%2$s/assets/images/headers/xili2017-h2-thumb.jpg',
				/* translators: added in child functions by xili */
				'description' => _x( '2017.2 by xili', 'header image description', 'twentyseventeen' ),
			),
		)
	);
	...
	$args = array(
		...
		'default-image' => '%2$s/assets/images/headers/xili2017-h1.jpg',
...
function twentyseventeen_xili_credits() {
	...
	printf(
		/* translators: added in child functions by xili */
		esc_html__( 'Multilingual child theme of Twenty Seventeen by %1$s and %2$s', 'twentyseventeen' ),
